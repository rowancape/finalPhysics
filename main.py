...
df = pd.read_csv("https://raw.githubusercontent.com/rowancape/finalPhysics/refs/heads/main/MainLinearAcceleration.csv")
gps = pd.read_csv("https://raw.githubusercontent.com/rowancape/finalPhysics/refs/heads/main/MainLocation.csv")
...
folium.PolyLine(locations=points, smooth_factor=10).add_to(m)
...
with st.echo():
  ...
  result = T * peakFreq
  st.markdown(f"> #### {T:.2f} × {peakFreq:.2f} ≈ {result:.0f}")
...
st.markdown(f"# Average lenght of step was {totalDist / 219:0.3f} meters")
